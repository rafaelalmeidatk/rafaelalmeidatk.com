...
While the Context API itself is simple, a bad implementation could make the maintanence of the code harder or degrade the performance of the app. I am going to share a few tips to help you organize and write more efficient contexts.
...
In order to determine if the value changed, the Context API uses the `Object.is()` function with the previous and new values. Since our value is an object it follows the referential equality rule, which means both values should have the same reference in order to be considered equal.
...
Re-rendering a couple components is not a big deal, but as your context grows the performance of the app may get worse since changing a single value will update a lot of unrelated components. If you notice that your context is holding two values that are completely unrelated to each other, consider spliting them into different contexts.

Are you noticing that your app is growing, there is too many contexts being created and managing the state is becoming complex? Maybe it is time to use a state management library, such as [Redux Toolkit](https://redux-toolkit.js.org) or [Recoil](https://recoiljs.org).
...
To fix this we are gonna change the default value to be `null`, so if the provider isn't found in the tree above the component we are gonna check for `null` and throw an error. This is easily done by changing the hook we made earlier:
...
Now if the consumer tries to use our hook without setting up the provider first, we are gonna give an useful error telling what is the problem. We could further improve the error with instructions:
...
This is the full code of the provider with all the tips applied, you can use it as a template for new providers:
